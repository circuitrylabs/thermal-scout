...
class TestEstimateThermalCost:
    ...
    def test_parameter_count_affects_thermal(self):
        ...
        for model_id, expected in test_cases:
            ...
            assert estimate_thermal_cost(model_info) == expected, (
...
class TestThermalSearch:
    ...
    @patch("thermal_scout.search.HfApi")
    def test_thermal_aware_search_sorts_by_efficiency(
        self, mock_hf_api_class, sample_model_list
    ):
        ...
        if high_count > 0:
            ...
            assert (
                thermal_costs.index("Low") < thermal_costs.index("High")
                if "High" in thermal_costs
                else True
            )

    @patch("thermal_scout.search.HfApi")
    def test_non_thermal_aware_preserves_order(
        self, mock_hf_api_class, sample_model_list
    ):
